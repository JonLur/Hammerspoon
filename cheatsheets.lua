    ...
    local menu = ""
        for pos,val in pairs(t) do
            if(type(val)=="table") then
                ...
                elseif(val['AXRole'] =="AXMenuItem" and not val['AXChildren']) then
                    --if( val['AXMenuItemCmdModifiers'] ~='0' and val['AXMenuItemCmdChar'] ~='') then
                    if( val['AXMenuItemCmdModifiers'] and val['AXMenuItemCmdChar'] ~='') then
                        if(type(val['AXMenuItemCmdModifiers'])=="table") then
                            for k, v in pairs( val['AXMenuItemCmdModifiers'] ) do
                              --print(k, v)
                              if(k) then
                                modifier = k
                              end
                        ...
                        menu = menu.."<li><div class='cmdModifiers'>"..commandEnum[modifier].." "..val['AXMenuItemCmdChar'].."</div><div class='cmdtext'>".." "..val['AXTitle'].."</div></li>"
                    end
                elseif(val['AXRole'] =="AXMenuItem" and type(val['AXChildren']) == "table") then
                    menu = menu..getAllMenuItems(val['AXChildren'][1])
                end
...
hs.hotkey.bind({"cmd", "alt", "ctrl"}, "C", "Shortkeys overview", function()
  if not myView then
